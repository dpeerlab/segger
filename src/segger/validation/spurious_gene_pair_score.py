...
def find_conditional_coexpression(adata: sc.AnnData) -> tuple[pd.DataFrame, dict[str, set[str]]]:
    ...
    return conditional_coexpression, cells_expressing_genes
...
def get_expansion_conditional_coexpression(transcripts: pd.DataFrame, nucleus_gdf: gpd.GeoDataFrame, buffer_size: float | pd.Series, genes: list[str] = None) -> pd.DataFrame:
    ...
    intersected_gdf.rename(columns={
        f'{index_name}_right': 'nucleus_boundaries_id_new',
        f'{index_name}': 'nucleus_boundaries_old'
    })
    ...
    normalized_counts_series = intersected_gdf.groupby('nucleus_boundaries_id_new').apply(
        normalize_nucleus_group
    ).rename("counts")

    logger.info("Normalization within buffered regions complete.")
    normalized_df = normalized_counts_series.reset_index()

    gene_to_nuclei_series = normalized_df.groupby('feature_name')['nucleus_boundaries_id_new'].apply(set)
...
def get_spurious_pairs_scores(nucleus_adata: sc.AnnData, transcripts: pd.DataFrame, nucleus_gdf: gpd.GeoDataFrame, buffer_size: float | pd.Series, return_nucleus_coexpression: bool = False) -> pd.DataFrame:
    nuclear_conditional_coexpression, _ = find_conditional_coexpression(nucleus_adata)
...
def get_spurious_pairs_scores_from_xenium(xenium_dir_path: os.PathLike) -> pd.DataFrame:
    ...
    buffer_size = 2.0  # TODO: CHANGE LATER!
    ...
    return spurious_pairs_scores, nuclear_conditional_coexpression
...
def get_segmentation_relative_spurious_scores(xenium_dir_path: os.PathLike, segmentation_adata: sc.AnnData, cutoff: float) -> pd.DataFrame:
    xenium_dir_path = Path(xenium_dir_path)
    spurious_pairs_scores, nuclear_conditional_coexpression = get_spurious_pairs_scores_from_xenium(xenium_dir_path)
    nuclear_conditional_coexpression, _ = find_conditional_coexpression(
        sc.read_parquet(xenium_dir_path / "processed/nucleus_adata.parquet")
    )

    spurious_pairs_scores = spurious_pairs_scores.copy()
    spurious_pairs = spurious_pairs_scores["log_conditional_coexpression"] > cutoff
    segmentation_normalized_df = _normalize_to_median(segmentation_adata)
    segmentation_conditional_coexpression, _ = find_conditional_coexpression(segmentation_adata, segmentation_normalized_df)
