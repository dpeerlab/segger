...
def _infer_polygon_max_vertices(existing_store: "zarr.Group", fallback: int) -> int:
    """Infer polygon vertex count from source template (vertices width / 2)."""
    try:
        polygon_sets = existing_store.get("polygon_sets")
        if polygon_sets is None:
            return fallback
        for key in ("1", "0"):
            if key in polygon_sets and "vertices" in polygon_sets[key]:
                width = int(polygon_sets[key]["vertices"].shape[1])
...
def seg2explorer(
    ...
    polygon_max_vertices : int
        Maximum number of vertices per polygon (including closure). If the source
        template has a different width, it will be overridden to match.
    ...
    # Open source store early to match template polygon width.
    existing_store, source_zarr_store = _open_source_cells_store(source_path)
    polygon_max_vertices = _infer_polygon_max_vertices(existing_store, polygon_max_vertices)
    ...
    cell_id_arr[:, 0] = np.array(cell_id, dtype=np.uint32)
    cell_id_arr[:, 1] = 1
    ...
    _create_like(set0, "cell_index", np.arange(n_cells, dtype=np.uint32), src0["cell_index"])
    ...
    _create_like(set1, "cell_index", np.arange(n_cells, dtype=np.uint32), src1["cell_index"])
