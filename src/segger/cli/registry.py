...
class ParameterRegistry:
    ...
    def register_from_file(self, file_path: str | Path, class_name: str, prefix: str | None = None) -> None:
        ...
        if docstring:
            ...
            doc_params = {
                param.arg_name: {"description": param.description or "", "type_name": param.type_name}
    ...
    def _check_conflicts(self, param_name: str, existing: ParameterInfo, new: ParameterInfo) -> None:
        ...
        if not existing.is_required and existing.default != new.default:
            raise ValueError(
                f"Parameter '{param_name}' has conflicting default values: " f"{existing.default} vs {new.default}"
            )

        # Check description conflict (only if both are non-empty)
        if existing.help and new.help and existing.help != new.help:
            raise ValueError(
                f"Parameter '{param_name}' has conflicting descriptions: " f"'{existing.help}' vs '{new.help}'"
            )

        # Check type annotation conflict (only if both are non-None)
        if (
            existing.type_annotation is not None
            and new.type_annotation is not None
            and existing.type_annotation != new.type_annotation
        ):
            raise ValueError(
    ...
    def get_parameter(self, param_name: str, **kwargs):
        ...
        **kwargs
        ...
        if self._framework != "cyclopts":
            raise ValueError(
                "get_parameter() is only available when framework='cyclopts'. " "Use get() instead for Typer."
            )
        ...
        if info.type_annotation is None:
            raise ValueError(f"Parameter '{param_name}' has no type annotation. " "Cannot create Annotated type.")

        # Create the Parameter with help and required, plus any user kwargs
        param_kwargs = {"help": info.help, "required": info.is_required}
