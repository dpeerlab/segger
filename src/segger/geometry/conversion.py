...
import cuspatial
import torch
from cuspatial.utils.column_utils import contains_only_points, contains_only_polygons
...
def points_to_coords(data: any) -> np.ndarray | cp.ndarray:
    ...
    """
    raise NotImplementedError(f"No implementation registered for type {type(data).__name__}")
...
def _(data: np.ndarray | cp.ndarray):
    if data.ndim != 2 or data.shape[1] != 2:
        raise ValueError(f"Input array must have shape (N, 2), but got {data.shape}.")
    return data
...
def _(data: torch.Tensor):
    if data.dim() != 2 or data.shape[1] != 2:
        raise ValueError(f"Input tensor must have shape (N, 2), but got {data.shape}.")
    if data.device == "cpu":
        return data.numpy()
...
def _(data: gpd.GeoSeries):
    if data.geometry.empty or data.geom_type.ne("Point").any():
        raise ValueError("Input must be a non-empty geopandas.GeoSeries of points.")
    coords = data.get_coordinates()
    if coords.shape[1] != 2:
        raise ValueError(f"Input must be points in 2 dimensions, but got {data.shape[1]}.")
    return coords
...
def _(data: cuspatial.GeoSeries):
    if data.empty or not contains_only_points(data):
        raise ValueError("Input must be a non-empty cuspatial.GeoSeries of points.")
    return data.points.xy.to_cupy().reshape(-1, 2)  # inherently 2D


# --- Points API ---
def points_to_geoseries(data: any, backend: Literal["geopandas", "cuspatial"]) -> gpd.GeoSeries | cuspatial.GeoSeries:
    ...
    if backend not in ["geopandas", "cuspatial"]:
        raise TypeError(f"Unsupported backend '{backend}'. Supported backends are " f"'geopandas' and 'cuspatial'.")
    # Passthrough
...
def _(data: torch.Tensor):
    if not data.is_nested or data.layout != torch.jagged:
        raise ValueError("Input tensor must be nested and have 'jagged' layout.")
    if data.dim() != 3 or data.shape[-1] != 2:
        raise ValueError("Input tensor must be of shape (N, j2, 2), but got {data.shape}.")
    if data.device == "cpu":
...
def _(data: gpd.GeoSeries):
    if data.geometry.empty or data.geom_type.ne("Polygon").any():
        raise ValueError("Input must be a non-empty geopandas.GeoSeries of polygons.")
    coords = data.get_coordinates()
...
def _(data: cuspatial.GeoSeries):
    if data.empty or not contains_only_polygons(data):
        raise ValueError("Input must be a non-empty cuspatial.GeoSeries of polygons.")
...
# --- Polygons API ---
def polygons_to_geoseries(data: any, backend: Literal["geopandas", "cuspatial"]) -> gpd.GeoSeries | cuspatial.GeoSeries:
...
def polygons_to_nested_tensor(
    ...
    return torch.nested.nested_tensor(splits, layout=torch.jagged, device=device)
