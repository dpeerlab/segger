...
class TileFitDataset(PartitionDataset):
    ...
    def _validate_geometry(
        ...
        if self.geometry_key not in node_store.node_attrs():
            raise AttributeError(f"{store_name} is missing '{self.geometry_key}' attribute.")
    ...
    def _validate_data(self, data: Data | HeteroData):
        ...
        if isinstance(data, Data):
            ...
            if "mask" in data:
                raise KeyError(f"{store_name} must not contain an attribute 'mask'.")
            ...
            raise TypeError(f"Input must be a PyG Data or HeteroData object, but got " f"{type(data).__name__}.")
    ...
    def _get_partition(self, data: Data | HeteroData) -> torch.Tensor:
        ...
        if isinstance(data, HeteroData):
            partition = {}
            for node_type in data.node_types:
                partition[node_type] = self.tiling.label(data[node_type][self.geometry_key])
            return partition
    ...
    def _mask_data(self, data: Data | HeteroData) -> Data | HeteroData:
        ...
        if isinstance(data, HeteroData):
            ...
            data["mask"] = self.tiling.mask(data[self.geometry_key], self.margin)
        return data
...
class TilePredictDataset(Dataset):
    ...
    def __init__(
        ...
        if self._is_hetero:
            ...
            if missing:
                raise ValueError(f"Missing 'pos' attribute for node type: " f"{', '.join(missing)}")
    ...
    def __getitem__(self, idx: int) -> Data | HeteroData:
        ...
        if idx < 0 or idx >= len(self):
            raise IndexError(f"Requested {idx}, but tiling only contains {len(self)} tiles.")
    ...
    def _subset(self, bounds: shapely.Polygon) -> Data | HeteroData:
        ...
        if self._is_hetero:
            ...
            subset = (
                ((pos[:, 0] >= outer[0]) & (pos[:, 0] < outer[2]) & (pos[:, 1] >= outer[1]) & (pos[:, 1] < outer[3]))
                .nonzero()
                .flatten()
            )
            sample = self.data.subgraph(subset)
