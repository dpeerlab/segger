...
def knn_to_edge_index(
    ...
    with torch.no_grad():
        ...
        deg = valid.sum(dim=1)
        index_ptr = torch.cat((torch.zeros(1, dtype=torch.long, device=device), deg.cumsum(0)))
...
def edge_index_to_knn(
    ...
    row = torch.repeat_interleave(torch.arange(B, device=neighbor_table.device), lengths)
...
def setup_segmentation_graph(
    ...
    """TODO: Add description."""
    tx_fields = TrainingTranscriptFields()
    return tx.with_row_index("_tid").filter(segmentation_mask).select(["_tid", tx_fields.cell_encoding]).to_torch().T
...
def setup_prediction_graph(
    ...
    # Shape-based graph
    points = tx[[tx_fields.x, tx_fields.y]].to_numpy()
    boundary_type = bd_fields.cell_value if mode == "cell" else bd_fields.nucleus_value
    ...
    return torch.tensor(result[["index_query", "index_match"]].values.T).to(torch.int).cpu()
