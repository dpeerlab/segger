...
from .utils import contours_to_polygons, masks_to_contours
...
def _preflight_checks(
    ...
    for pat in [
        ...
        try:
            ...
            msg = f"No file or directory with pattern '{pat}' " f"found in {data_dir}."
            raise FileNotFoundError(msg)

    fov_info = pd.read_csv(next(data_dir.glob(fields.fov_positions_filename)), index_col="FOV")
    required_cols = {"X_mm", "Y_mm"}
    missing_cols = required_cols - set(fov_info.columns)
    if missing_cols:
        raise ValueError(f"Missing columns in FOV info: {', '.join(missing_cols)}")

    expected_fovs = [str.zfill(str(fov), 3) for fov in fov_info.index]
    expected_files = lambda prefix: {f"{prefix}_F{fov_id}.tif" for fov_id in expected_fovs}
    ...
    for dirname, prefix in [
        ...
        if missing:
            raise FileNotFoundError(f"Missing {len(missing)} {prefix} TIFFs:\n" + "\n".join(sorted(missing)))
