...
def get_quadtree_kwargs(
    ...
    kwargs = {"max_depth": 10, "max_size": 10000}
...
def get_expression_matrix(
    ...
    poly_quad_pairs = cuspatial.join_quadtree_and_bounding_boxes(quadtree, bboxes, **kwargs)
...
def get_buffered_counts(
    ...
    boundaries = read_parquet_region(
        filepath_boundaries, BoundaryColumns.x, BoundaryColumns.y, bounds=outset, extra_columns=[BoundaryColumns.id]
    ).compute()
...
def get_transcripts_regions(
    ...
    regions = quadtree_df.loc[~quadtree_df["is_internal_node"], ["x_min", "y_min", "x_max", "y_max"]]
    return regions
...
def get_buffered_counts_distributed(
    ...
    futures = client.map(lambda q: get_transcripts_regions(filepath_transcripts, bounds=q), quadrants)
