...
class FastTripletSelector:
    ...
    @torch.no_grad()
    def sample_triplets(self, labels: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
...
class TripletLoss(TripletMarginLoss):
    ...
    def __init__(self, cluster_similarity: torch.Tensor, margin: float = 1.0, **kwargs) -> None:
