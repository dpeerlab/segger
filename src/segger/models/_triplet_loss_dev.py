...
class FastTripletSelector:
    ...
    @torch.no_grad()
    def _build_index(self, labels: torch.Tensor) -> None:
    ...
    @torch.no_grad()
    def stratified_sample_anchors(self, labels: torch.Tensor) -> torch.Tensor:
    ...
    @torch.no_grad()
    def sample_triplets(self, labels: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        self._build_index(labels)
        ...
        pos_pres = torch.searchsorted(self._cdf_pos[pres_idx], uni_pos.unsqueeze(-1)).squeeze(-1)
        ...
        neg_pres = torch.searchsorted(self._cdf_neg[pres_idx], uni_neg.unsqueeze(-1)).squeeze(-1)
        ...
        dists = 1.0 - self.similarity.to(labels.device)
    ...
    def sample_triplets_stratified(
        self, labels: torch.Tensor, anchor_indices: torch.Tensor
        ...
        dists = 1.0 - self.similarity.to(device)
...
class TripletLoss(TripletMarginLoss):
    ...
    def __init__(
        self, cluster_similarity: torch.Tensor, margin: float = 1.0, stratified: bool = True, **kwargs
    ) -> None:
    ...
    def forward(self, embeddings: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        ...
        if self.stratified:
            ...
            positives, negatives, _, _ = self.selector.sample_triplets_stratified(labels, anchor_indices)
...
class MetricLoss:
    ...
    def forward(self, embeddings: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        ...
        return mse_loss(cos_pos, 1 - dists_pos.to(torch.float), reduction="mean") + mse_loss(
            cos_neg, 1 - dists_neg.to(torch.float), reduction="mean"
        )
